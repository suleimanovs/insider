...
JUnit называют "юнит-тестовым" фреймворком, но **Unit и JUnit — не одно и то же**. JUnit — это инструмент. Unit — это стиль.
И чтобы понимать, как писать хорошие тесты, нужно сначала увидеть, как сам JUnit эволюционировал — от простых процедур к полноценной платформе.

---

### История JUnit: от процедур до DSL

**JUnit 3** (2000-е) был процедурным. Тесты писали в виде классов, которые расширяли `TestCase`, а методы начинались с `test`.
...
Тест автоматически находился по имени (`test...`) — никакой магии.

---

**JUnit 4** (2006) ввёл аннотации и убрал наследование. Больше не нужно было расширять `TestCase`, а методы помечались `@Test`.
...
Это дало гибкость: появились `@Before`, `@After`, `@Ignore` и кастомные раннеры. Но тест всё ещё был процедурой.

---

**JUnit 5** (2017+) — это уже не просто "юнит-фреймворк". Это **платформа**, разделённая на:

* **JUnit Platform** — движок запуска (Gradle, IDE, CI);
* **JUnit Jupiter** — новая модель API (лямбды, `@Nested`, `@DisplayName`);
* **JUnit Vintage** — поддержка старых тестов (JUnit 3 и 4).

Теперь можно писать **параметризованные**, **иерархические** и **динамические** тесты:
...
Сейчас `@Test` — это **входная точка тестовой модели**. Она говорит: *"это не просто функция — это контракт".*

Что это значит?

* Метод должен быть **чистым**: без зависимостей от других тестов.
* Он может быть **запущен отдельно**.
* Его поведение должно быть **предсказуемым** (иначе флак).

Аннотация `@Test` отделяет **исполняемую документацию** от просто utility-функций. Это то, на что IDE, CI и разработчик могут положиться.

---

### Unit ≠ JUnit

Unit-тест — это **изолированный** тест, где все внешние зависимости подменены. Он не зависит от JUnit. Его можно написать вообще без аннотаций и фреймворков.

```kotlin
fun testAdditionLogic() {
    val calculator = Calculator()
    val result = calculator.add(2, 3)
    check(result == 5) // никаких @Test
}
```

JUnit — это **инфраструктура**. Он запускает, репортит, агрегирует, фильтрует. Он позволяет писать не только unit, но и:

* интеграционные,
* сквозные (end-to-end),
* property-based тесты,
* performance/timeout/parallel tests.
