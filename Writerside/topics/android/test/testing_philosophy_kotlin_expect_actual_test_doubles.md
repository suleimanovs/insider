...
Такое разделение позволяет выстраивать баланс: большая часть тестов должна находиться на нижних уровнях ради скорости и покрытия, а верхние — фиксировать критические пользовательские сценарии и визуальное качество.

#### Unit tests

Модульные тесты — это фундамент любого процесса тестирования. Их задача проста в формулировке и сложна в реализации: убедиться, что каждая минимальная единица программы — функция, метод или класс — выполняет свои обязанности корректно и предсказуемо. Важно уточнить термин: слово «модульные» здесь относится к самостоятельной единице кода, а не к модулю сборки или библиотеке. Если представить систему как здание, то unit-тесты проверяют прочность каждого кирпича до того, как они будут объединены в стены и этажи.
...
* **Property-based testing** — формулировка универсальных свойств функций и их проверка на случайных данных.
* **Boundary testing** — фокус на крайние значения и нештатные входные данные.
* **Mutation testing** — искусственное внесение ошибок в код для проверки того, что тесты действительно обнаруживают сбои.

Цель unit-тестов — обеспечить надёжность базовой логики, чтобы более высокие уровни тестирования концентрировались на взаимодействиях и пользовательских сценариях.
...
Ключевая особенность — отсутствие полной изоляции. Здесь допустимо участие файловой системы, базы или даже сетевых запросов (чаще в контролируемом окружении). Главное — не уходить в масштаб всей системы, а ограничиться проверкой конкретного взаимодействия. Такие тесты выполняются дольше unit-тестов, но дают уверенность, что компоненты корректно связаны.
...
* **Contract testing** — проверка, что сервисы общаются по согласованному контракту (например, клиент и сервер используют один формат API).
* **Compatibility testing** — тестирование работы модуля с разными версиями зависимостей или внешних сервисов.
* **Integration with in-memory DB** — ускорение тестов за счёт использования лёгких in-memory реализаций вместо полноценных серверов.

#### End-to-End tests

Термин *End-to-End* буквально переводится как «от конца до конца» и означает прохождение полного пути — от начального действия пользователя до конечного результата в системе. Такой тест не ограничивается проверкой одной функции или даже пары модулей: он прогоняет сценарий через весь стек — интерфейс, бизнес-логику, базу данных и внешние сервисы.
...
* **Regression testing** — повторная проверка ранее работающих сценариев после изменений в коде.
* **Smoke testing** — минимальный набор тестов, которые подтверждают, что система запускается и выполняет основные функции.
* **Sanity testing** — точечная проверка конкретного изменения или исправления бага.
...
* **Visual regression testing** — сравнение текущего интерфейса со скриншотами эталонной версии.
* **Accessibility testing** — проверка доступности интерфейса для людей с ограниченными возможностями (чтение экраном, контрастность, наличие меток).
* **Cross-browser / cross-device testing** — проверка одинаковой работы интерфейса на разных устройствах и в разных средах.
...
Когда мы обсуждаем пирамиду или трофей, речь идёт о том, **сколько** тестов должно быть на каждом уровне. Но распределение — это только часть картины. Сам тест может существовать на правильном уровне и при этом не давать доверия. Причина в том, что важна не только форма набора тестов, но и качество каждого из них.

Чтобы тест приносил пользу, он должен подчиняться ряду принципов: изоляция, детерминированность, повторяемость и независимость. Эти принципы универсальны — они одинаково применимы и к unit-тестам, и к интеграционным, и к e2e.

* **Изоляция.** Тест управляет своим окружением и не полагается на неконтролируемые факторы. База данных, сетевые вызовы, глобальное состояние — всё это должно быть под контролем или заменено на предсказуемые аналоги. В противном случае результат теста будет зависеть не от кода, а от случайностей среды.

* **Детерминированность.** Один и тот же тест при неизменном коде всегда должен давать один и тот же результат. Если тест иногда проходит, а иногда падает без изменений в логике, это подрывает доверие ко всей системе тестирования. Для детерминированности устраняют случайность, фиксируют источники времени и делают входные данные воспроизводимыми.

* **Повторяемость.** Тест можно запускать сколько угодно раз — локально у разработчика, на CI-сервере или в другой среде — и результат всегда останется одинаковым. Это критично для непрерывной интеграции, где один и тот же набор проверок должен срабатывать сотни раз в день.

* **Независимость.** Тесты не влияют друг на друга. Каждый сам подготавливает данные и очищает за собой окружение. Если корректность одного теста зависит от запуска другого, вся система становится хрупкой: падение одного метода тянет за собой каскад ошибок.
