...
сериализуются одним и тем же апи, под капотом проверка первым делом падает на Externalizable, если бы мы использовали один и тот же модель
и для Serializable и для Externalizable, при сериализаций всегда использовался бы флоу для Externalizable, по этому мы создали две одинаковые
...
В этом классе есть некоторые изменения, для Java Serializable и Externalizable мы немного переделали, убрав промежуточное создание 
файла, так как при передачи между процессами и компонентами в андроид нет промежуточного слоя для записи в фаил, из-за того что Parcelable
работает именно с байтами, то Java Serializable и Externalizable  так же будут работать с массивами байт что бы сравнение было точным и 
справедливым. В примпере с kotlinxSerializable мы так же переделали, используем не Json формат, а ProtoBuff, опять же из принципа справедливости и точности,
так как Json это все же формат текстового рода, по этому используем ProtoBuf. Далее результаты теста:
