...
Раз мы определились с базисом, теперь перейдем постепенно к способам сериализаций в порядке эвололюций, с точки зрения Kotlin и Android, с
точки зрения Java эволюций не было, первое рещение и остается последним.

## Интерфейс Serializable
В мире JVM, Serializable пожалуй является самым первым и частым способом сериализаций, сам Serializable является интерфейсом маркером, то есть
не имеющий никакой логики в себе, далее глянем на его исходники:
```java
package java.io;
...
```
Для того что бы мы могли сериализовать объект, нам достаточно наследоваться от этого интерфейса:
...
Теперь попробуем сериалозвать объект данного класса, используя OutputStream:
```kotlin
...
На выходе мы получим фаил serialization.bin с сериализлванными объектами, если попробуем прочесть этот фаил как текстовый формат, то есть .txt, то увидим следующее:
```text
�� sr PersonTZ���c I dateOfBirthL addresst Ljava/lang/String;L nameq ~ xp  �t New Yorkt John Doe
```
Конечно же этот фаил для нас не читаем, и кодировка UTF-8 видимо не подходящая, но все же можно увидеть названия полей, их типы, названия класса, и даже значения включая название пакетов.
Вы можете задаваться вопросам, если сериализация это превращение объекта в байты, то почему мы не видим нули и единицы а видим плюс минус читаемый текст? то все дело в том что мы открыли фаил как text, то за нас уже произашла некоторая декодировка при превращений байтов в текст.
Теперь нас интерусует откуда взялась вся информация о классе этого объекта который сериалозован, и тут мы узнаем что Serializable полностью работает
...
Далее мы создаем фаил в корне текущего проекта, и задавая имя serialization.bin,
После этого мы создаем FileOutputStream, его работа лишь в том что бы открыть нам возможность записывать что либо
в наш фаил, представтье его как ручку, на этом этапе и до этого этапа еще нет никакой сериализаций, и реффлексиий.

Реффлексия и магия Serialazablе проявлятеся с момента создания ObjectOutputStream, а если быть точнее, то с момент
вызова функций writeObject, вся работа происходит именно в нем, что он делает? Берет наш объект класса Person, и постарается его сериализовать, а затем записать в наш фаил serialization.bin
Сам ObjectOutputStream на самом деле нечего не знает о нашем фаиле serialization.bin, ObjectOutputStream работает с FileOutputStream, вот FileOutputStream и является посредником между нашим фаилом и объектом.

Интересный момент, большинство видов Stream-ов, в лице наследников InputStream и OutputStream являются декораторами, например FileOutputStream и сам ObjectOutputStream всего лиш декораторы.

Процесс десериализаций с Serializable будет выглядит таким образом:
```kotlin
...
Здесь мы сначала находим фаил и затем прокидываем его в FileInputStream что бы сохроненный набор байтов
обратно десерилизовать и восстановить из него объекта Person.
...
Этот метод своего рода Provider, если от класса унаследовались и переопределили открыть метод для  пеореопределения, то будет использована
логика наследника, иначе вызов передается методу `writeObject0` который является стандатным у `ObjectOutputStream`, в нашем случае мы не наследовались от ObjectOutputStream
по этому мы попадаем в метод `writeObject0`:
...
та логика которая в конечном итоге отвечает за работу сериализаций стандартного объекта. 

Но мы так же решили специально оставить создание объекта ObjectStreamClass, так как он играет важную роль в сериализаций.
...
По сути, это сериализационный аналог реффлексии, но не на уровне исполнения, а на уровне протокола(контракта).
...
Если нет — создается новый. В процессе создания выполняется полный анализ структуры класса:
...
Таким образом, `ObjectStreamClass.lookup` — это точка перехода от уровня кода к уровню протокола сериализации. 
До этого момента JVM работала с объектом как с экземпляром типа, после — как с набором описанных структур и байтов, имея всю инфоормацию о том
кто перед ним сейчас, сам по себе этот вызов в первый раз занимает очень много времени, и памяти, это первый залп использования реффлексий для сериализаций.
...
> Любой еnum в JVM является Serializable по умолчанию, все потому что все еnum классы не явно наследуются от настоящего класса java.lang.Enum. А класс java.lang.Enum уже наследуется от интерфейса Serializable
...
Первое, что делает `writeOrdinaryObject`, — вызывает `desc.checkSerialize()`.
...
Механизм Object Serialization в Java использует фиксированный бинарный протокол, где каждый элемент (объект, класс, поле, массив и т.д.) предваряется своим маркером. Для объекта это `0x73` — байт `TC_OBJECT`. Таким образом, запись объекта всегда начинается с этого маркера.

Следом вызывается `writeClassDesc(desc, false)`.
В этом месте сериализация переходит от конкретного экземпляра к описанию его класса. Метод `writeClassDesc` отвечает за запись дескриптора класса — структуры, содержащей имя класса, `serialVersionUID`, количество и типы сериализуемых полей, а также ссылки на суперклассы.
...
Нас интересует вызов writeSerialData, он вызывается в случае если объект не record, не externalizable, а именно serializable , метод `writeSerialData`, на который в большинстве случаев и замыкается сериализация. Если `writeOrdinaryObject` можно назвать маршрутизатором, то `writeSerialData` — исполнитель, именно здесь происходит фактическая запись состояния объекта.

В начале метода формируется массив `slots`, получаемый через `desc.getClassDataLayout()`. Это внутреннее представление иерархии классов, участвующих в сериализации. Каждый элемент массива — это `ClassDataSlot`, содержащий ссылку на `ObjectStreamClass` конкретного уровня наследования. Таким образом, `slots` задаёт строгий порядок обхода цепочки классов сверху вниз, от самого предка, объявившего сериализуемые поля, до конечного потомка.
