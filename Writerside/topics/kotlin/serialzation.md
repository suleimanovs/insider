...
```
> В зависимости от версии андроид размер и количество байтов может отличаться
...
Интересное наблюдение: размер 116 байт значительно больше, чем мы ожидали. Почему? `Parcel` записывает полное квалифицированное имя класса включая package. Строки хранятся в UTF-16, что удваивает размер для ASCII символов. Если бы мы использовали `writeToParcel()` напрямую без записи имени класса через `writeParcelable()`, размер был бы меньше - около 50-60 байт только для данных.

Это показывает важный момент: `Parcel` действительно оптимизирован для IPC, где полное имя класса нужно для корректной десериализации, но не для минимального размера данных. В контексте Intent между Activity это не проблема - данные живут микросекунды в памяти. Но это еще один аргумент против использования `marshall()` для хранения - формат содержит platform-specific детали вроде полных имен классов.
...
Одна аннотация `@Serializable`, и класс готов к сериализации. Синтаксис напоминает `@Parcelize`, но работает иначе и на всех платформах. Обратите внимание на результат: это не бинарный формат с метаданными из `Serializable`, не flat buffer из `Parcel`, а чистый JSON. 67 байт против 130-150 у `Serializable` и 56 у `Parcel`.

Но JSON - это только один из форматов. Помните, мы говорили про модульность? Тот же класс можно сериализовать в Protocol Buffers, CBOR, или даже XML. Давайте посмотрим на разницу:
...
val json = Json.encodeToString(person)
println("JSON: $json")
println("Размер: ${json.toByteArray().size} байт")

val protobuf = ProtoBuf.encodeToByteArray(Person.serializer(), person)
println("ProtoBuf размер: ${protobuf.size} байт")

val cbor = Cbor.encodeToByteArray(Person.serializer(), person)
println("CBOR размер: ${cbor.size} байт")
```

Вывод покажет:
```
JSON: {"name":"John Wick","dateOfBirth":1964,"address":"New York"}
Размер: 67 байт
ProtoBuf размер: 31 байт
CBOR размер: 42 байта
```

JSON читаемый, но занимает больше места - 67 байт. Protocol Buffers - компактный бинарный формат, даёт всего 31 байт, даже меньше чем 56 байт у `Parcel`! CBOR, формат похожий на MessagePack, занимает промежуточные 42 байта. Выбор формата зависит от задачи: для API используйте JSON, для мобильного кеша или сетевых протоколов - Protobuf или CBOR, для конфигураций - JSON или YAML.
