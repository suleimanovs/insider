...
// JSON (читаемый, но занимает больше места)
val json = Json.encodeToString(person)
println("JSON: $json")
...
// CBOR (компактный бинарный формат, похожий на MessagePack)
val cbor = Cbor.encodeToByteArray(Person.serializer(), person)
println("CBOR размер: ${cbor.size} байт")
// Размер: 42 байта
```

Protocol Buffers даёт 31 байт - даже меньше, чем 56 байт у `Parcel`! CBOR занимает 42 байта. JSON больше всех (67 байт), но зато читаем человеком и универсален. Выбор формата зависит от задачи: для API используйте JSON, для мобильного кеша или сетевых протоколов - Protobuf или CBOR, для конфигураций - JSON или YAML.
