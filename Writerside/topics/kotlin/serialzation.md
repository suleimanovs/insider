...
```
Конечно же, этот файл для нас не читаем из-за бинарного формата, и кодировка UTF-8 видимо не подходящая. Тем не менее, в выводе можно разглядеть названия полей, их типы, название класса и даже значения полей, включая информацию о пакетах.

Вы можете задаться вопросом: если сериализация представляет собой превращение объекта в байты, то почему мы видим не нули и единицы, а относительно читаемый текст? Дело в том, что мы открыли файл как текстовый, и текстовый редактор уже выполнил некоторую декодировку, пытаясь интерпретировать байты как символы.

Но откуда же взялась вся эта информация о классе нашего сериализованного объекта? Здесь начинается самое интересное: `Serializable` полностью работает с помощью Reflection API. Прежде чем углубиться в детали рефлексии, давайте разберемся в логике приведенного выше примера.

Рассмотрим пошагово, что происходит в коде:

1. Сначала мы создаем объект класса `Person` для John Wick (да, да, того самого)
2. Затем создаем файл `serialization.bin` в корне текущего проекта
3. После этого создаем `FileOutputStream`. Его задача — открыть нам возможность записывать данные в файл. Можно представить его как ручку для письма. На этом этапе никакой сериализации или рефлексии еще не происходит.
...
Важно понимать архитектуру: сам `ObjectOutputStream` на самом деле ничего не знает о нашем файле `serialization.bin`. Вместо этого `ObjectOutputStream` работает с `FileOutputStream`, который и является посредником между файлом и объектом. Это классический паттерн проектирования "Декоратор" (Decorator).

На самом деле, большинство видов Stream-ов (наследники `InputStream` и `OutputStream`) являются декораторами. Например, как `FileOutputStream`, так и `ObjectOutputStream` представляют собой декораторы, добавляющие дополнительную функциональность к базовому потоку.
...
Процесс десериализации работает зеркально: мы сначала находим файл и затем передаем его в `FileInputStream`, чтобы восстановить объект `Person` из сохраненного набора байтов.

### Роль интерфейса Serializable

Давайте убедимся, что интерфейс `Serializable` действительно необходим. Если мы уберем у класса `Person` наследование от интерфейса `Serializable`, то получим такую ошибку:
```
...
### Внутреннее устройство сериализации
...
Обратите внимание на важную строку, которую мы специально оставили в коде: создание объекта `ObjectStreamClass`. Этот объект играет ключевую роль во всем процессе сериализации.

#### Создание дескриптора класса

Рассмотрим подробнее вызов `ObjectStreamClass.lookup(cl, true)`. Именно здесь начинается реальная работа сериализации. 
...
#### Детальный разбор writeSerialData

Нас особенно интересует третья ветвь — обычная сериализация через `Serializable`. Давайте рассмотрим метод `writeSerialData` более подробно:
...
Мы подробно рассмотрели внутреннее устройство механизма сериализации через интерфейс `Serializable` в Java. Начав с простого примера использования, мы прошли через все уровни абстракции: от пользовательского API до низкоуровневых деталей реализации, включая работу с рефлексией, дескрипторами классов и бинарным протоколом сериализации. Это понимание станет фундаментом для дальнейшего изучения более современных подходов к сериализации в экосистеме Kotlin и Android.
