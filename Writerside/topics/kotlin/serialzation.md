...
Мы разобрались, почему Android не мог использовать ни `Serializable`, ни `Externalizable`. Первый слишком медленный и создает избыточную нагрузку на GC, второй не интегрирован с Binder и несет багаж Java сериализационного протокола. Требовался механизм, который будет писать данные напрямую в плоский бинарный буфер без типизации, минимизировать создание временных объектов и работать на уровне нативного кода через JNI.

`Parcelable` решает эти задачи. Интерфейс требует реализации двух методов и статического поля CREATOR:
...
Ручная реализация на Kotlin требовала значительного количества boilerplate кода:
...
Для трех полей требуется почти 40 строк кода. Малейшая ошибка в порядке записи и чтения приводит к трудноотлавливаемым багам.

Android Studio предоставила готовый шаблон для генерации кода через Alt+Insert (Cmd+N на Mac), но проблема оставалась: при изменении структуры класса необходимо вручную обновлять методы сериализации.

С появлением Kotlin плагин `kotlinx-android-extensions` добавил аннотацию `@Parcelize`, которая автоматически генерирует всю реализацию во время компиляции:
...
Плагин `kotlinx-android-extensions` включал не только `@Parcelize`, но и синтетические импорты для view (`import kotlinx.android.synthetic.main.*`), которые позже признали антипаттерном. Плагин разделили, и `@Parcelize` переехала в отдельный модуль `kotlin-parcelize`:
...
Генерация происходит на уровне компилятора без runtime библиотек. `Parcel` это тонкая Java обертка над нативной структурой данных на C++. Класс `android.os.Parcel` работает через JNI:
...
Нативная реализация в C++ (frameworks/native/libs/binder/Parcel.cpp) работает напрямую с памятью. Строка конвертируется в UTF-16, добавляется ее длина, и данные пишутся в линейный буфер без промежуточных объектов.

При передаче через `Intent.putExtra()` буфер передается через Binder driver в ядре Linux с использованием copy-on-write. На принимающей стороне создается `Parcel` с указателем на буфер памяти. В отличие от JVM сериализации с множеством слоев абстракции, здесь практически прямая работа с памятью.

Компилятор генерирует для `@Parcelize` следующий код:
...
Порядок записи и чтения должен быть идентичен: `writeString`, `writeInt`, `writeString` при записи, и `readString`, `readInt`, `readString` при чтении. Это критически важно, так как `Parcel` это untyped buffer без метаданных о типах. При вызове `parcel.readInt()` просто читаются следующие 4 байта из буфера и интерпретируются как integer. Если порядок не совпадает, будут прочитаны некорректные данные.

С `@Parcelize` компилятор гарантирует корректность порядка. Kotlin-parcelize корректно обрабатывает nullable поля:
...
`Parcel.writeString()` при получении null записывает специальный маркер (значение -1 как длина строки), а `readString()` при чтении возвращает null. Это встроенная поддержка nullable типов на уровне `Parcel`, реализованная до появления Kotlin.

Метод `describeContents()` в большинстве случаев возвращает 0. Исключение составляют объекты, содержащие file descriptor: для них необходимо вернуть `Parcelable.CONTENTS_FILE_DESCRIPTOR`, чтобы Binder корректно передал системные ресурсы через ядро:
...
Флаг `flags` в методе `writeToParcel` обычно игнорируется, но может содержать `Parcelable.PARCELABLE_WRITE_RETURN_VALUE`, указывающий, что объект передается как возвращаемое значение из Binder транзакции.

Вложенные `Parcelable` объекты поддерживаются:
...
При записи `Person` вызывается `parcel.writeParcelable(address, flags)`, который в свою очередь вызывает `address.writeToParcel()`. При чтении вызывается `parcel.readParcelable<Address>(Address::class.java.classLoader)`. Рекурсия работает на уровне компилятора без runtime накладных расходов.

Для коллекций `Parcel` имеет специальные оптимизированные методы:
...
`writeStringList` сначала записывает размер списка, затем каждую строку по очереди. `readStringList` читает размер и строки. Аналогично работают `writeIntArray`, `writeParcelableList`, `writeMap` и другие специализированные методы.

`@Parcelize` работает только с классами, где все свойства объявлены в primary конструкторе. Если у вас есть свойство вне конструктора, оно не будет сериализовано:
...
Компилятор генерирует код на основе primary конструктора и не может знать о дополнительных полях.

Для кастомизации процесса сериализации можно реализовать интерфейс `Parceler`:
