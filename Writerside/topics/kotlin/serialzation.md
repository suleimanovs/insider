...
файла, так как при передаче между процессами и компонентами в Android нет промежуточного слоя для записи в файл, из-за того что Parcelable
работает именно с байтами, то Java Serializable и Externalizable так же будут работать с массивами байт, чтобы сравнение было точным и 
справедливым. В примере с kotlinxSerializable мы так же переделали, используем не Json формат, а ProtoBuf, опять же из принципа справедливости и точности,
так как Json это все же формат текстового рода, поэтому используем ProtoBuf. Далее результаты теста:
